...
/*************************************************************************************************/
WarGrey::STEM::Histogramlet::Histogramlet(float width, float height, uint32_t box_hex, uint32_t mark_hex)
        : width(flabs(width)), height(flabs(height)), box_color(box_hex), mark_color(mark_hex) {
    if (this->height == 0.0F) {
...
void WarGrey::STEM::Histogramlet::set_box_color(uint32_t hex) {
    if (this->box_color != hex) {
        this->box_color = hex;
        this->clear_geometry();
        this->notify_updated();
    }
...
void WarGrey::STEM::Histogramlet::set_mark_color(uint32_t hex) {
    if (this->mark_color != hex) {
        this->mark_color = hex;
        this->clear_geometry();
        this->notify_updated();
    }
