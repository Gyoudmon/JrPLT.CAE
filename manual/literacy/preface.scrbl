...
的积木到角色的代码区；那这个操作在 C++/Python 中也应该简化为一行代码：比如 @tt{ball.bounce()}。
学习 Scratch 的学生不关心“碰到边界反弹”的积木是怎么实现的，那他们在初学 C++/Python 时也不需要深究
@tt{bounce()} 函数的实现算法，而有机会在本系列课程的后续学习中逐渐深入。学有余力的学生也可以自己研读
配套的代码库。
