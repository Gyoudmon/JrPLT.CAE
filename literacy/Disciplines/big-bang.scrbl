...
@handbook-scenario{阶段1:试听与准备}

在此阶段，师生通过简单程序互相了解，
也包括安装开发工具、熟悉程序源码的基本结构、
初步认识程序语言中的基本概念等。

考虑到学生可能是小学在读，或者仅有零星的信息技术知识。
一方面，传统的 @litchar{Hello, World!} 开局对学生和教师都没有代入感；
一方面，鼠标操作的客观复杂度越来越高，
让学生直接在开发软件中用鼠标操作文件和文件夹，
反而会显著降低整体教学进度。
因此，我们的开局从认识 Shell 开始，
这是一种为专业人士设计的命令行工具集。
实际上，Shell 也是一种程序语言，
而且自带 @emph{REPL}(@emph{Read-Evaluate-Print-Loop})，
因而可作为@tech{自顶向下}过程的最顶层(一行 Shell 胜过万行 C/C++)。
从实际授课情况上看，输入几个单词就能导致文件浏览器里的内容发生变化，
这个效果本身能带给潜在的学生很丰富的惊喜感，
甚至不亚于小朋友第一次看到神奇的化学实验。

在安装好必要的开发软件、观摩了课程作品之后，
学生会直接开始学习《函数和对象》，
在此过程中也会顺便理解变量、
类型和值等最基础且最重要的概念。
方法上会同时同语文/英语和数学/物理两个角度切入，
直接把编程与语、数、英的关系明确地揭示给学生看。
...
@handbook-scenario{阶段2:新手上路寓教于乐}

在此阶段，本课程将直接进入图形化和游戏化的学习，
基本目标是让学生熟悉英文键盘、
一定程度上激发继续学习的兴趣。
...
随机是概率论中的重要概念，
而概率论是关于常识的数学模型。
也就是说，随机现象在自然界和日常生活中都极为普遍，
这些现象常常会被比喻成“醉汉的脚步”。
以上就是设计本阶段课程情景的理论基础，
但对于孩子而言，醉汉很可能并不那么容易碰到，
因此，可以通过一个简单的游戏来模拟：蒙上眼睛走路
(走得好的同学先原地转几圈再走)。

该理论和情景可用于解释学生考试成绩的波动性。
比如，设定一个较高的目标得分之后，
每次考试可能进步几分、保持不变，甚至退步几分，
这些都是正常现象，属于随机游走；
倘若成绩总体上确实是在不断接近目标，
那这个随机游走就转化为了醉汉漫步。
