...
void WarGrey::STEM::ShapeCosmos::load(float width, float height) {
    ...
    this->circle = self->insert_one(new Circlelet(50, PINK, PURPLE));                   // 紫边粉红色圆
...
void WarGrey::STEM::ShapeCosmos::reflow(float width, float height) {
    // 排列以组合三角形
    self->move_to(this->lines[0], 200, 400, GraphletAnchor::LB); // 左下角对齐
    self->move_to(this->lines[1], 200, 400, GraphletAnchor::LT); // 左上角对齐
    self->move_to(this->lines[2], 600, 400, GraphletAnchor::RB); // 右下角对齐

    // 排列长方形和椭圆
    self->move_to(this->rectangle, 800, 100, GraphletAnchor::LT);
    self->move_to(this->circle, 900, 400, GraphletAnchor::CC);
    self->move_to(this->ellipse, 900, 600, GraphletAnchor::CC);

    // 排列正多边形
    for (int i = 0; i < sizeof(this->regular_polygons) / sizeof(IShapelet*); i++) {
        self->move_to(this->regular_polygons[i], 100 * i, 750, GraphletAnchor::CC);
    }
}
